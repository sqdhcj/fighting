...
public interface SysAuthService
{
    ...
    /**
     * 查找全部权限（为zTree准备的集合）,并且和角色对应的权限进行比较，
     * 以便判断是否展开和勾选，默认传入的roleid为‘nouuid’
     * @return List<Ztree>
     */
    List<Ztree> findALLToZtree(String roleid);

    /**
     * 添加节点的子节点；1）首先根据节点id查询到对应的节点信息；
     * 2）再根据该节点信息和新的节点名称进行名称组合，以该组合名称查询对应的节点信息是否存在；
     * 3）如果新节点信息不存在，则保存该新节点
